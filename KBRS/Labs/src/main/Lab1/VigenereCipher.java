...
public class VigenereCipher {
    ...
    private static final double[] ENGLISH_CHAR_FREQ_ARRAY = {
    ...
    private static void createGUI() {
        ...
        encryptItem.addActionListener(e -> {
            ...
            try {
                ...
                str = encrypt(originalTextArea.getText(), key);
        ...
        decryptItem.addActionListener(e -> {
            ...
            try {
                ...
                str = decrypt(originalTextArea.getText(), key);
                resulTextArea.setText(new String(str));
                keyArea.setText(key);
            } catch (UnsupportedEncodingException e1) {
                // TODO Auto-generated catch block
    ...
    private static String getKey(int keyLength, String text) {
        ...
        double freq[] = new double[ENGLISH_CHAR_AMOUNT];
        int length = text.length();
        int amount, indexC, intC;
        double max;
        char curC;
        StringBuffer key = new StringBuffer();
        for (int k = 0; k < keyLength; k++) { // keyLength алфавитов, для каждого свои частоты
            for (int i = 0; i < ENGLISH_CHAR_AMOUNT; i++) {
                freq[i] = 0;
            }
            max = -1;
            indexC = -1;
            amount = (length / keyLength) + (((length % keyLength) > k) ? 1 : 0);
            for (int i = k; i < length; i += keyLength) { // по тексту смотрим частоты для каждого i-ого через keyLength символа
                curC = text.charAt(i);
                for (int j = 0; j < ENGLISH_CHAR_AMOUNT; j++) {
            ...
            for(int m = 0; m < ENGLISH_CHAR_AMOUNT; m++){
                System.out.println(m + ":= " + freq[m] / ((double) length / keyLength));
            }
